...
def _copy_build_source(destination: Path, *, legacy: bool) -> None:
    ...
    if legacy:
        ...
        content = content.replace('version = "0.4.1"', 'version = "0.3.0"', 1)
...
def test_legacy_candidate_must_be_uninstalled_before_helixweave(
    ...
    assert current_wheel.name == "helixweave-0.4.1-py3-none-any.whl"
...
assert metadata.version("helixweave") == "0.4.1"
try:
    metadata.version("encode-pipeline")
except metadata.PackageNotFoundError:
    pass
