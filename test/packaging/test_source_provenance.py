...
def _write_installed_distribution(
    ...
    *,
    include_inventory: bool = True,
    metadata_directory: str = "helixweave-0.4.1.dist-info",
) -> None:
    package = site_packages / "encode_pipeline"
    package.mkdir(parents=True)
    package_files = {
        "encode_pipeline/__init__.py": b'__version__ = "0.4.1"\n',
        "encode_pipeline/runtime.py": b'ARTIFACT_KIND = "installed"\n',
    }
    for relative_path, raw in package_files.items():
        (site_packages / relative_path).write_bytes(raw)
        ...
        json.dumps(
            {
                ...
                "url": "file:///artifact/helixweave-0.4.1-py3-none-any.whl",
            }
        ),
        encoding="utf-8",
    )
    if include_inventory:
        ...
        for relative_path, raw in package_files.items():
            ...
            package_records.append(
                f"{relative_path},sha256={digest.rstrip('=')},{len(raw)}"
            )
        (metadata / "RECORD").write_text(
            "\n".join(
                (
                    *package_records,
                    "helixweave-0.4.1.dist-info/METADATA,,",
                    "helixweave-0.4.1.dist-info/top_level.txt,,",
                    "helixweave-0.4.1.dist-info/direct_url.json,,",
                    "helixweave-0.4.1.dist-info/RECORD,,",
                )
            )
            + "\n",
            encoding="utf-8",
...
def test_checkout_mode_rejects_two_physical_claimants_with_the_same_version(
    ...
    _write_distribution(
        ...
        metadata_directory="HELIXWEAVE-copy-0.4.1.dist-info",
    )
...
def test_checkout_mode_rejects_a_pth_file_symlink_escape(tmp_path: Path) -> None:
    ...
    site_packages = tmp_path / "site-packages"
    _write_distribution(site_packages, source_root=current)
    pth = site_packages / "__editable__.helixweave-0.4.1.pth"
...
def test_checkout_mode_fails_stably_for_damaged_record(tmp_path: Path) -> None:
    ...
    metadata = site_packages / "helixweave-0.4.1.dist-info"
    metadata.mkdir()
    (metadata / "METADATA").write_text(
        "Metadata-Version: 2.4\nName: helixweave\nVersion: 0.4.1\n",
        encoding="utf-8",
...
def test_installed_mode_ignores_unrecorded_bytecode_cache_files(
    tmp_path: Path,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    cache = site_packages / "encode_pipeline" / "__pycache__"
    cache.mkdir()
...
def test_installed_mode_accepts_a_canonical_recorded_console_script(
    tmp_path: Path,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    script = python.parent / "helixweave-record-probe"
    ...
    record = site_packages / "helixweave-0.4.1.dist-info" / "RECORD"
...
def test_installed_mode_rejects_a_missing_record_owned_package_file(
    tmp_path: Path,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    missing_file = site_packages / "encode_pipeline" / "runtime.py"
    missing_file.unlink()

    result = _run_installed_guard(python)

    assert result.returncode == 2
    assert "[installed_artifact_integrity_invalid]" in result.stderr
    assert str(missing_file) not in result.stderr
    assert "runtime.py" not in result.stderr
    assert "Traceback" not in result.stderr


def test_installed_mode_rejects_a_record_owned_package_hash_mismatch(
    tmp_path: Path,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    corrupted_file = site_packages / "encode_pipeline" / "runtime.py"
    corrupted_file.write_bytes(b'ARTIFACT_KIND = "corrupted"\n')

    result = _run_installed_guard(python)

    assert result.returncode == 2
    assert "[installed_artifact_integrity_invalid]" in result.stderr
    assert str(corrupted_file) not in result.stderr
    assert "runtime.py" not in result.stderr
    assert "Traceback" not in result.stderr


@pytest.mark.parametrize("filename", ("unrecorded.py", "unrecorded.pyc"))
def test_installed_mode_rejects_an_unrecorded_importable_package_file(
    tmp_path: Path,
    filename: str,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    unrecorded_file = site_packages / "encode_pipeline" / filename
    unrecorded_file.write_text(
        "raise AssertionError('must never import')\n",
        encoding="utf-8",
    )
    ...
    assert result.returncode == 2
    assert "[installed_artifact_integrity_invalid]" in result.stderr
    assert str(unrecorded_file) not in result.stderr
...
def test_installed_mode_rejects_an_invalid_record_structure(
    tmp_path: Path,
    malformation: str,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    ...
    if malformation == "two-columns":
        rows[rows.index(package_row)] = package_row.rsplit(",", 1)[0]
    elif malformation == "duplicate-path":
        rows.append(package_row)
    elif malformation == "unsafe-relative-path":
        rows[rows.index(package_row)] = package_row.replace(
            "encode_pipeline/runtime.py",
            "encode_pipeline/../runtime.py",
        )
    else:
        external_file = tmp_path / "private-record-target"
        relative_external = Path(
            os.path.relpath(external_file, site_packages)
        ).as_posix()
        rows.append(f"{relative_external},,")
    record.write_text("\n".join(rows) + "\n", encoding="utf-8")
    ...
    assert result.returncode == 2
    assert "[installed_artifact_integrity_invalid]" in result.stderr
    assert str(record) not in result.stderr
    assert "runtime.py" not in result.stderr
    assert "Traceback" not in result.stderr


def test_installed_mode_rejects_a_record_owned_package_symlink_escape(
    tmp_path: Path,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    recorded_file = site_packages / "encode_pipeline" / "runtime.py"
    external_file = tmp_path / "private-runtime.py"
    recorded_file.replace(external_file)
    recorded_file.symlink_to(external_file)

    result = _run_installed_guard(python)

    assert result.returncode == 2
    assert "[installed_artifact_integrity_invalid]" in result.stderr
    assert str(recorded_file) not in result.stderr
    assert str(external_file) not in result.stderr
    assert "runtime.py" not in result.stderr
    assert "Traceback" not in result.stderr


def test_installed_mode_rejects_an_unrecorded_non_regular_package_entry(
    tmp_path: Path,
) -> None:
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    fifo = site_packages / "encode_pipeline" / "unrecorded.fifo"
    os.mkfifo(fifo)
    ...
    assert "unrecorded.fifo" not in result.stderr
    assert "Traceback" not in result.stderr
...
def test_installed_mode_rejects_noneditable_directory_install(
    ...
    python, site_packages = _create_venv(tmp_path / "venv")
    _write_installed_distribution(site_packages)
    (site_packages / "helixweave-0.4.1.dist-info" / "direct_url.json").write_text(
        json.dumps(
            {
                "dir_info": {},
